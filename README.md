...
| **问题排查/** | |
| [问题排查/场景题与故障排查.md](问题排查/场景题与故障排查.md) | CPU飙高/OOM/超时/慢查询/缓存一致性（5专题）|
...
- [x] 线上CPU飙高如何排查 → [场景题与故障排查.md](问题排查/场景题与故障排查.md#一线上-cpu-飙高如何排查)
- [x] 内存溢出如何排查 → [场景题与故障排查.md](问题排查/场景题与故障排查.md#二内存溢出如何排查)
- [x] 接口超时如何排查 → [场景题与故障排查.md](问题排查/场景题与故障排查.md#三接口超时如何排查)
- [x] 数据库慢查询优化 → [场景题与故障排查.md](问题排查/场景题与故障排查.md#四数据库慢查询优化)
- [x] 缓存与数据库数据一致性 → [场景题与故障排查.md](问题排查/场景题与故障排查.md#五缓存与数据库数据一致性)

#### 10.2 线上问题监控与全维度排查 ⭐⭐⭐
- [x] 排查方法论（MELT 四维模型 / 黄金 5 步法 / 止血原则）→ [线上问题监控与排查.md](问题排查/线上问题监控与排查.md#一线上问题排查方法论-)
- [x] CPU 异常排查（user/sys/iowait/负载/Arthas 火焰图）→ [线上问题监控与排查.md](问题排查/线上问题监控与排查.md#二cpu-异常排查-)
- [x] 内存异常排查（系统内存/JVM 堆/GC/泄漏 SOP）→ [线上问题监控与排查.md](问题排查/线上问题监控与排查.md#三内存异常排查-)
- [x] 磁盘异常排查（空间/Inode/IO 瓶颈/已删除未释放）→ [线上问题监控与排查.md](问题排查/线上问题监控与排查.md#四磁盘异常排查-)
- [x] 网络异常排查（连接状态/CLOSE_WAIT/丢包/DNS/抓包）→ [线上问题监控与排查.md](问题排查/线上问题监控与排查.md#五网络异常排查-)
- [x] 文件句柄排查（FD 泄漏/ulimit/lsof）→ [线上问题监控与排查.md](问题排查/线上问题监控与排查.md#六文件句柄fd异常排查-)
- [x] 进程与线程排查（僵尸进程/线程泄漏/上下文切换/死锁）→ [线上问题监控与排查.md](问题排查/线上问题监控与排查.md#七进程与线程排查-)
- [x] JVM 专项监控（GC 日志/jcmd/jstat/jstack/jmap 大全）→ [线上问题监控与排查.md](问题排查/线上问题监控与排查.md#八jvm-专项监控-)
- [x] 日志排查技巧（grep/awk/sed/日志规范）→ [线上问题监控与排查.md](问题排查/线上问题监控与排查.md#九日志排查技巧-)
- [x] 监控体系搭建（Prometheus/Grafana/Exporter/告警规则）→ [线上问题监控与排查.md](问题排查/线上问题监控与排查.md#十监控体系搭建-)
- [x] 综合排查实战（RT 升高/5xx/内存泄漏/磁盘满 4 场景）→ [线上问题监控与排查.md](问题排查/线上问题监控与排查.md#十一综合排查实战场景-)
- [x] 常用命令速查表 & 排查口诀 → [线上问题监控与排查.md](问题排查/线上问题监控与排查.md#十二常用命令速查表-)

#### 10.3 Java服务与中间件排查 ⭐⭐⭐（NEW）
- [x] Java 服务 CPU 飙高排查（6步流程/7大根因/实战命令）→ [Java服务与中间件排查.md](问题排查/Java服务与中间件排查.md#一java-服务-cpu-飙高排查)
- [x] Java 服务内存飙高排查（6步流程/6大泄漏场景/代码示例）→ [Java服务与中间件排查.md](问题排查/Java服务与中间件排查.md#二java-服务内存飙高排查)
- [x] 接口响应慢全链路排查（6步流程/Arthas trace/watch/monitor）→ [Java服务与中间件排查.md](问题排查/Java服务与中间件排查.md#三接口响应慢全链路排查)
- [x] MySQL 性能排查（CPU飙高/内存飙高/慢查询 SOP/监控指标）→ [Java服务与中间件排查.md](问题排查/Java服务与中间件排查.md#四mysql-性能排查)
- [x] Redis 性能排查（CPU飙高/内存飙高/慢响应/Big Key）→ [Java服务与中间件排查.md](问题排查/Java服务与中间件排查.md#五redis-性能排查)
- [x] Arthas 完全指南（安装/命令速查/6大实战场景/生产注意事项）→ [Java服务与中间件排查.md](问题排查/Java服务与中间件排查.md#六arthas-完全指南)
- [x] 综合实战案例（订单接口慢/Redis延迟/FGC泄漏/MySQL CPU 100%）→ [Java服务与中间件排查.md](问题排查/Java服务与中间件排查.md#七综合实战案例)
